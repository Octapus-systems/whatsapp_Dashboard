...
Wirebot uses a database to store:
...
Wirebot supports two database backends that can be selected at deployment time:
...
Wirebot v0.2+ implements a **dual-database architecture** that separates boot configuration from user data:
...
When using PostgreSQL Built-in mode, Wirebot automatically:
...
Wirebot provides endpoints for migrating data between database types:
...
To ensure date/time values work across both SQLite and PostgreSQL, Wirebot uses a `DateTransformer` that stores dates as ISO 8601 text strings:
...
# Create backup
pg_dump -Fc $DB_NAME > $BACKUP_DIR/wirebot_$DATE.dump

# Compress
gzip $BACKUP_DIR/wirebot_$DATE.dump

# Upload to S3 (optional)
aws s3 cp $BACKUP_DIR/wirebot_$DATE.dump.gz s3://backups/wirebot/
