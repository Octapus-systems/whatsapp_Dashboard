...
export class WhatsAppWebJsAdapter extends EventEmitter implements IWhatsAppEngine {
  ...
  /**
   * Maps @lid identifiers (e.g. 129725225808006@lid) to canonical @c.us JIDs
   * (e.g. 919745482646@c.us). Populated on session ready and updated lazily
   * whenever a new @lid is resolved via getContact().
   */
  ...
  private setupEventHandlers(): void {
    ...
    this.client.on('ready', () => {
      ...
      // Eagerly build the LID → JID map so the first incoming message from a
      // @lid sender can be resolved instantly without an extra getContact() call.
      void (async () => {
        try {
          ...
          this.logger.log(`[LID] Built LID→JID map with ${mapped} entr${mapped === 1 ? 'y' : 'ies'}`);
        } catch (e) {
          this.logger.warn('[LID] Failed to build LID→JID map on ready', String(e));
        }
      })();
    });
    ...
    this.client.on('message', async msg => {
      try {
        // ── LID resolution ──────────────────────────────────────────────────
        // WhatsApp now sends messages from @lid senders (Linked Device IDs)
        // instead of the classic @c.us phone-number JID. We must canonicalise
        // the chatId to @c.us so the DB and frontend can match against contacts.
        let resolvedChatId: string = msg.from;
        if (msg.from.endsWith('@lid')) {
          // Fast path: check prebuilt map populated at session-ready
          const cached = this.lidToJidMap.get(msg.from);
          if (cached) {
            resolvedChatId = cached;
            this.logger.log(`[LID] Fast-resolved ${msg.from} → ${resolvedChatId}`);
          } else {
            // Slow path: ask WhatsApp Web.js for the contact behind this LID
            try {
              const contact = await msg.getContact();
              const jid = contact?.id?._serialized;
              if (jid && !jid.endsWith('@lid')) {
                resolvedChatId = jid;
                // Cache so subsequent messages from same sender are instant
                this.lidToJidMap.set(msg.from, jid);
                this.logger.log(`[LID] Slow-resolved ${msg.from} → ${resolvedChatId}`);
              } else {
                this.logger.warn(`[LID] Could not resolve ${msg.from} — keeping @lid as chatId`);
              }
            } catch (lidErr) {
              this.logger.warn(`[LID] getContact() failed for ${msg.from}`, String(lidErr));
            }
          }
        }
        // ────────────────────────────────────────────────────────────────────
        ...
        if (((msg.type as string) === 'vcard' || (msg.type as string) === 'contact_card') && msg.body) {
          ...
          for (const line of lines) {
            if (line.startsWith('FN:')) {
              ...
              if (match) {
                ...
                if (parts.length > 1) {
                  number = parts[parts.length - 1].replace(/[^\d+]/g, '');
                }
              }
            }
          }
  ...
  async getContacts(): Promise<Contact[]> {
    ...
    // serialised JID so that the API always returns a unique-keyed list.
