...
export class MessageService {
  ...
  async sendAudio(sessionId: string, dto: SendMediaMessageDto): Promise<MessageResponseDto> {
    ...
    const message = await this.saveOutgoingMessage(sessionId, {
      ...
      metadata: {
        media: {
          mimetype: dto.mimetype || 'audio/mp3',
          filename: dto.filename,
          url: dto.url,
          data: dto.base64,
        },
  ...
  /**
   * Save incoming message (from WhatsApp contact/user) to the database.
   * Sets direction=INCOMING and status=SENT explicitly to avoid relying on DB defaults.
   */
